...
namespace Steam
{
    ...
    struct SteamUser
    {
        ...
        EBeginAuthSessionResult BeginAuthSession(const void *pAuthTicket, int cbAuthTicket, SteamID_t steamID)
        {
            ...
            Callbacks::Completerequest(Callbacks::Createrequest(), Callbacks::Types::ValidateAuthTicketResponse_t, Request);
        ...
        HAuthTicket GetAuthSessionTicket(void *pTicket, int cbMaxTicket, uint32_t *pcbTicket)
        {
            ...
            *pcbTicket = 7;
            ...
            Callbacks::Completerequest(Callbacks::Createrequest(), Callbacks::Types::GetAuthSessionTicketResponse_t, Request);
        ...
        {
            // WTF?
            assert(cbDataToInclude >= 0);

            // Append game data.
            Authticket.resize(cbDataToInclude + 32);
            std::memcpy(Authticket.data() + 32, pDataToInclude, cbDataToInclude);
            ...
            Callbacks::Completerequest(RequestID, Callbacks::Types::EncryptedAppTicketResponse_t, Request);
            return RequestID;
        }
        ...
        uint32_t GetVoiceOptimalSampleRate() { return 1024 * 32; }
        ...
        {
            const auto Request = new Callbacks::StoreAuthURLResponse_t();
            const auto RequestID = Callbacks::Createrequest();

            Infoprint(va("%s: %s", __FUNCTION__, pchRedirectURL));
            std::strncpy(Request->m_szURL, "ayria.se", 512);

            Callbacks::Completerequest(RequestID, Callbacks::Types::StoreAuthURLResponse_t, Request);
            return RequestID;
        }
        SteamAPICall_t GetMarketEligibility()
        {
            const auto Request = new Callbacks::MarketEligibilityResponse_t();
            const auto RequestID = Callbacks::Createrequest();
            Request->m_bAllowed = true;

            Callbacks::Completerequest(RequestID, Callbacks::Types::MarketEligibilityResponse_t, Request);
            return RequestID;
        }
        ...
        SteamAPICall_t GetDurationControl()
        {
            ...
            const auto Request = new Callbacks::DurationControl_t();
            const auto RequestID = Callbacks::Createrequest();
            Request->m_eResult = EResult::k_EResultOK;
            Request->m_appid = Global.ApplicationID;
            Request->m_bApplicable = false;

            Callbacks::Completerequest(RequestID, Callbacks::Types::DurationControl_t, Request);
            return RequestID;
        }
